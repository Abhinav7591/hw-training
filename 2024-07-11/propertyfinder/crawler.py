...
class PropertyFinderCrawler:
    def __init__(self):
        ...
        self.error_collection = self.mongo_pipeline.db["Propertyfinder_crawler_error_urls"]
        self.category_collection = self.mongo_pipeline.db["Propertyfinder_category_urls"]
        ...
        for subcategory_url in subcategory_urls:
            ...
            while True:
                ...
                if response.status_code == 200:
                    ...
                    property_urls = selector.xpath("//article[@class ='property-card-module_property-card__wrapper__ZZTal ']/a/@href").extract()
                    ...
                    for property_url in property_urls:
                        try:
                            self.collection.insert_one({'subcategory_url': subcategory_url, 'property_url': property_url})
                            logging.info(f"Inserted URL: {property_url}")
                        except Exception as e:
                            logging.error(f"Error inserting URL {property_url}: {e}")
