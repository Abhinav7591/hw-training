...
class fressnapfParser:
    def __init__(self):
        ...
        # self.collection = get_mongo_collection("fressnapf", "product_urls")
        # self.parsed_collection = product_details_collection("fressnapf","product_details")
    ...
    def parse_product_page(self, url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                grammage_unit = " "
                if title_text:
                    numerical_value_match = re.search(r'\b(\d+x\d+(?:,\d+)?|\d+(?:,\d+)?)\s*([lkg]+)', title_text)
                ...
                product_info = {
                    ...
                    'feeding_recommendation' : feeding_data,
                    'warranty' : warranty,
                    'color' : color,
