...
class fressnapfParser:
    def __init__(self):
        ...
        # self.collection = get_mongo_collection("mueller_site", "product_urls")
        # self.parsed_collection = product_details_collection("mueller_site","product_details")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if price_per_unit:
                    ...
                    match = re.search(r'(\d[\d\s,.]*€\s*/\s*\d+\s*\w+)', price_per_unit)
                    if match:
                        price_per_unit = match.group(1).replace('\xa0', ' ')
                    else:
                        price_per_unit = ''
                else:
                    price_per_unit = ''
                ...
                product_str_match = re.search(r'product: \'(.*?)\',\n', script_text, re.DOTALL)
                if product_str_match:
                    product_str = product_str_match.group(1)
                    product_json_str = bytes(product_str, "utf-8").decode("unicode_escape")
                    image_data = json.loads(product_json_str)

                    image_urls = [asset['image']['zoom'] for asset in image_data['assets'] if 'image' in asset and 'zoom' in asset['image']]
